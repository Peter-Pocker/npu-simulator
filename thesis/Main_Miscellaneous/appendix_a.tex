...
\begin{itemize}
  \item 核阵列：\texttt{num\_cores\_x=2}，\texttt{num\_cores\_y=2}，即 4 个核心。
  \item 数据位宽：\texttt{element\_size\_bits=8}（INT8）。
  \item 单核：\texttt{mac\_units=256}，\texttt{vector\_units=64}，\texttt{clock\_freq\_mhz=1000}，\texttt{use\_analytical\_time=false}（由模拟器根据张量形状与微架构推算计算周期）。
  \item SRAM：\texttt{size\_kb=512}，读写带宽 64 bytes/cycle。
  \item 网络接口：\texttt{max\_outstanding\_reqs=16}，\texttt{injection\_queue\_size=8}，\texttt{ejection\_queue\_size=8}。
  \item NoC：\texttt{backend="booksim2"}，接入 BookSim2 周期级网络模拟器。\texttt{flit\_size\_bytes=16}，\texttt{num\_vcs=8}（虚拟通道数），\texttt{vc\_buf\_size=8}（每虚拟通道缓冲深度）。
  \item DRAM：\texttt{backend="ramulator2"}，接入 Ramulator2 周期级存储模拟器。\texttt{standard="DDR4"}，\texttt{timing="DDR4\_2400R"}，\texttt{num\_channels=4}，\texttt{num\_ranks=2}，\texttt{scheduler="FRFCFS"}。
  \item 拓扑：\texttt{dram\_controllers=[]} 表示 DRAM 直连各核（不经 NoC）。
\end{itemize}

完整 JSON 如下（与 \texttt{configs/full\_config.json} 一致）：
...
本课题工作量主要包括：（1）周期级多核 NPU 模拟器的设计与实现，包括核心状态机、SRAM 与依赖建模、NoC/DRAM 抽象接口及 BookSim2/Ramulator2 的集成；（2）Gemini 编译器 Scheduler IR 的解析与驱动逻辑，将调度结果转化为可执行的 workload 序列与数据依赖；（3）实验脚本与可视化工具链，涵盖 ZeBu trace 解析、仿真对比、瓶颈分解与论文图表生成；（4）配置体系与实验设计，包括多种芯片配置、微基准 IR 与真实负载实验流程。除模拟器主体外，脚本与辅助工具保证实验可复现、结果可追溯。下表给出自写代码（含 C++ 源码、头文件、Python 脚本与 Shell 脚本）的规模统计，总规模约一万二千行。
...
\begin{table}[H]
  ...
  \begin{tabular}{llr}
    ...
    \midrule
    核心模拟器（\texttt{src/} + \texttt{include/npu\_sim/}） & 状态机、SRAM、IR 解析、引擎与主程序 & 约 9200 \\
    ...
    \midrule
    \textbf{合计} & 自写代码（不含第三方与子模块） & \textbf{约 11900} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
